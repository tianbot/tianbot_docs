...
function buildKnowledgeSidebar(
  ...
  const sensors = source.basicItems.find((item) =>
    item.items?.some((child) =>
      child.items?.some((grandchild) => grandchild.link === '/basic/camera/mono-usb-camera')
      || child.items?.some((grandchild) => grandchild.link === '/basic/lidar/2d-ydlidar'),
    ),
  );
            ...
            items: [
              ...
              },
              {
                ...
                items:[
                  ...
                  {text: 'Nokov 动捕将 VRPN 流发布到 ROS1 广播中', link: '/tianbot/development/vrpn_client_ros_with_nokov'}
                ]
              },
